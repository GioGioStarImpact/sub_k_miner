# -*- coding: utf-8 -*-
"""CLI runner: Verilog → IR → AIG → frequent subgraph mining (vendor‑neutral)."""
...
def patterns_to_matrix_rows(patterns):
    ...
    for rank, (key, pat) in enumerate(items, 1):
        ...
        for idx in range(1<<k):
            row = {"rank": rank, "count": pat.count, "k": k, "m": m,
                   "canon_outs_hash": f"{(hash(outs)&0xffffffff):08x}", "idx": idx}
...
def main():
    ...
    ap.add_argument("--matrix_csv", action="store_true")
    args = ap.parse_args()

    vtext = Path(args.verilog).read_text(encoding="utf-8", errors="ignore")
    ir_per_module = mm.parse_verilog_to_ir_per_module(vtext)
    Path(f"{args.out_prefix}_ir_per_module.json").write_text(json.dumps(ir_per_module, indent=2), encoding="utf-8")

    global_pats, per_mod = mm.mine_across_modules(
        ir_per_module, dmax=args.dmax, kmax=args.kmax, mmax=args.mmax, min_support=args.min_support
    )
    ...
    csv_path = f"{args.out_prefix}_global.csv"
    json_path = f"{args.out_prefix}_global.json"
    ...
    if args.matrix_csv:
        ...
        matrix_csv_path = f"{args.out_prefix}_global_matrix.csv"
